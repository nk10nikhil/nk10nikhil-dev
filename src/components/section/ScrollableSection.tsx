...
const ScrollableSection = ({ className }: ScrollableSectionProps) => {
  ...
  useEffect(() => {
    const handleParallax = () => {
      ...
      imagesRef.current.style.transform = `translateY(${
        window.scrollY * 0.1
      }px)`;
    };
  ...
  return (
    <div
      className={`min-h-screen bg-white text-gray-900 dark:bg-gray-900/10 dark:text-white ${
        className || ""
      }`}
    >
      <div className="container mx-auto max-w-7xl px-4 pt-5">
        <div className="flex flex-col lg:flex-row">
          ...
          <div className="hidden lg:flex lg:w-1/2 sticky top-32 h-[600px]">
            <div
              ref={imagesRef}
              className="image-container w-full floating-animation"
            >
              {sections.map((section, index) => (
                <div
                  key={section.id}
